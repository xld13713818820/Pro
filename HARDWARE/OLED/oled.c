...
const u8 F16x16_Idx[] = 
{
	"测距离周长面积度高坡亩仪搜索卫星大小查记录删除系统值设单价公顷元询每电池压经纬°㎡型号版本剩余空间返回退出徐林东的"
};
...
const u8 F16x16[] = 
{
	...
	0x01,0x00,0xFF,0x00,0x20,0x11,0x0D,0x41,0x81,0x7F,0x01,0x05,0x09,0x31,0x00,0x00,//徐
	
	0x10,0x10,0xD0,0xFF,0x90,0x10,0x00,0x10,0x10,0xD0,0xFF,0xD0,0x10,0x10,0x10,0x00,
	0x04,0x03,0x00,0xFF,0x00,0x11,0x08,0x04,0x03,0x00,0xFF,0x00,0x03,0x04,0x08,0x00,//林"
	...
	0x00,0x7F,0x21,0x21,0x21,0x21,0x7F,0x00,0x00,0x00,0x43,0x80,0x40,0x3F,0x00,0x00,//的"
...
const u8 F8X16[]=
{
	...
	0x00,0x38,0x38,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,//°C95
...
void OLED_IO_Init(void)
{
  ...
  GPIO_PinRemapConfig(GPIO_Remap_SWJ_Disable,ENABLE);      //SWJ 完全失能
...
void LCD_Init(void)        
{
  ...
  LCD_WrCmd(0x81);//--set contrast control register  设置对比度控制寄存器
...
//==============================================================
//函数名： void LCD_Put_Column(u8 x,u8 y,u32 data)
//功能描述：操作一列显示，一列全显示0XFF,清除一列0X00;
//参数：真实坐标值(x,y),x的范围0～127，y的范围0～64
//返回：无
//==============================================================
void LCD_Put_Column(u8 x,u8 y,u32 data)
{	
	LCD_WrCmd(0xb0+(y>>3));
	LCD_WrCmd(((x&0xf0)>>4)|0x10);
	LCD_WrCmd((x&0x0f)|0x00);
